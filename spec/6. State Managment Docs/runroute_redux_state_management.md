  ...
  addWaypoint: (waypoint) =>
    set((state) => ({
      currentRoute: state.currentRoute
        ? {
            ...state.currentRoute,
            waypoints: [...state.currentRoute.waypoints, waypoint].sort(
              (a, b) => a.order - b.order
            ),
          }
        : null,
...
export const useUserProfile = (userId?: string) => {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api
        .get<UserProfile[]>(`/users?id=eq.${targetUserId}`)
        .then((data) => data[0]),
...
export const useUserStatistics = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      api.post("/rpc/get_user_statistics", {
...
export const useSaveRoute = () => {
  ...
  return useMutation({
    mutationFn: (route: Omit<PlannedRoute, "id" | "createdAt">) =>
      api.post<PlannedRoute>("/planned_routes", {
        ...route,
        user_id: user?.id,
      }),
...
export const useRuns = () => {
  ...
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 0 }) =>
      api.get<CompletedRun[]>(
        `/runs?user_id=eq.${user?.id}&order=started_at.desc&limit=20&offset=${pageParam}`
      ),
...
export const useRunDetails = (runId: string) => {
  return useQuery({
    ...
    queryFn: () =>
      api.get<CompletedRun[]>(`/runs?id=eq.${runId}`).then((data) => data[0]),
    enabled: !!runId,
  });
};
...
export const useDeleteRun = () => {
  ...
  return useMutation({
    mutationFn: (runId: string) => api.delete(`/runs?id=eq.${runId}`),
...
export const useLikePost = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (isLiked) {
        return api.delete(
          `/post_likes?post_id=eq.${postId}&user_id=eq.${
...
export const usePostComments = (postId: string) => {
  return useQuery({
    ...
    queryFn: () =>
      api.post<Comment[]>("/rpc/get_post_comments", { post_id: postId }),
    enabled: !!postId,
  });
};
...
export const useFollowUser = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, { userId }) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["social", "connections", useAuthStore.getState().user?.id],
      });
    },
  });
};
...
const LiveTrackingScreen = () => {
  ...
  return (
    <View>
      ...
      <Text>
        Duration: {Math.floor(metrics.durationSeconds / 60)}:
        {metrics.durationSeconds % 60}
      </Text>
      <Text>
        Pace: {Math.floor(metrics.currentPaceSecondsPerKm / 60)}:
        {Math.floor(metrics.currentPaceSecondsPerKm % 60)}/km
      </Text>
